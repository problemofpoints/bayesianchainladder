...
- **Notebooks are generated.** Edit `docs/notebooks/build_modus_operandi.py`, `docs/notebooks/build_evw_2019.py` or `docs/notebooks/build_ev_2006.py`, not the `.ipynb`; rebuild and re-execute (see `docs/notebooks/README.md`). `tests/test_notebooks.py` executes all three under `--run-slow`. The latter two compare this package's output against Peter England's own published figures, transcribed into `bayesianchainladder/data/england_reference_values.json` (see its `_provenance` block and `bayesianchainladder/data/README.md`); `tests/test_reference_values.py` is a fast check of the same JSON against the library's analytic/bootstrap output, independent of building the notebooks.
