"""14_glm_priors_mt5cal.py — Mine MT5_cal posteriors for per-spec t-family GLM prior elicitation.

Fits incremental ~ 1 + (1 | origin) + bs(dev_idx, df=4) + (1 | calendar)
with family='t', link='identity', response_per_exposure=True (loss-ratio scale)
and exposure='net_earned_premium', on the same 24 sampled triangles per line
used in 02_glm_model_comparison.py.
...
Output: cache/mt5cal_posteriors.parquet
Run: uv run python references/prior-elicitation-2026/14_glm_priors_mt5cal.py
"""
...
def _fit_and_extract(triangle, seed: int) -> dict:
    """Fit MT5_cal (t, identity, loss-ratio, RE origin + calendar) and extract posteriors."""
    ...
    # 1|origin sigma (random-intercept SD on origin RE).
...
def _result_path() -> Path:
    return cache_path("mt5cal_posteriors.parquet")
        ...
        for snl_id in snl_ids:
            ...
            try:
                ...
                print(
                    ...
                    f"nu_mean={res['nu_mean']:.2f}  "
                    f"origin_sigma={res['origin_sigma_median']:.4f}  "
                    f"calendar_sigma={res['calendar_sigma_median']:.4f}  "
                    f"({elapsed:.0f}s)",
                    flush=True,
                )
